...
class Processor(Initializer):
    ...
    def train(self, epoch):
        ...
        for num, (x, y, _) in enumerate(train_iter):
            ...
            if self.no_progress_bar:
                ...
                train_iter.set_description('Loss: {:.4f}, LR: {:.6f}'.format(loss.item(), lr))
    ...
    def eval(self):
        ...
        if self.args.score:
            ...
            score_dict = dict(
                zip(self.feeders['eval'].name, score))
            if not os.path.exists('./score/{}_score'.format(self.model_name)):
                os.makedirs('./score/{}_score'.format(self.model_name))
            with open('./score/{}_score/score.pkl'.format(self.model_name), 'wb') as f:
    ...
    def start(self):
        ...
        if self.args.evaluate:
            ...
            checkpoint = U.load_checkpoint(self.args.work_dir, self.args.config, self.model_name, self.args.pretrained_path, self.args.use_pretrained)
            if checkpoint:
