...
def load_checkpoint(work_dir, config_name, model_name='resume',pretrained_path="", use_pretrained=False):
    if model_name == 'resume':
        ...
        benchmark_name = config_name.rsplit('_',1)[0]
        file_name = '{}/{}/{}/{}.pth.tar'.format(pretrained_path, benchmark_name, config_name, model_name)
        logging.info('Evaluating with pretrained model: {}!'.format(file_name))
    else:
        dirs, accs = {}, {}
        work_dir = '{}/{}'.format(work_dir, config_name)
...
def set_logging(args):
    if args.debug or args.evaluate or args.extract or args.visualize or args.generate_data:
        ...
        save_dir = '{}/{}/{}'.format(args.work_dir, args.config, ct)
